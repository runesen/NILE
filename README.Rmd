...
The `NILE` estimator can be used to estimate a nonlinear causal influence of a single predictor $X$ on a real-valued response $Y$. 
It exploits an instrumental variable (IV) setting with a single exogenous variable $A$ which serves as instrument. 
Predictions obtained from the NILE estimator are causal predictions in the sense that they predict the values 
of $Y$ under interventions which break the dependence between $X$ and possible confounders of $(X,Y)$. 
For further details refer to the paper @rune2020 [https://arxiv.org/abs/2006.07433].
